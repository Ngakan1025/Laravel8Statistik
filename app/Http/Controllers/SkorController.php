...
class SkorController extends Controller
{
    ...
    {
        $skor = Skor::all();
        $maxSkor = Skor::max('skor');
        $minSkor = Skor::min('skor');
        $rata2 = number_format(Skor::average('skor'),3);
        // $data = [
        //     'skor' => $this->Skor->allData()
        // ];
       
        //untuk tabel frekuensi
        $frekuensi = Skor::select('skor', DB::raw('count(*) as frekuensi'))  //ambil skor, hitung banyak skor taruh di tabel frekuensi
                                ->groupBy('skor')                              //urutkan sesuai skor
                                ->get();
        $totalskor = Skor::sum('skor');              
        $totalfrekuensi = Skor::count('skor');        //karena total frekuensi = banyaknya skor yang ada

        return view('skor', ['skor' => $skor,
                            'max' => $maxSkor, 
                            'min' => $minSkor, 
                            'rata2' => $rata2,
                            'frekuensi' => $frekuensi,
    ...
    public function insert()
    {
        ...
        $this->Skor->addData($data);
        return redirect()->route('skor')->with('pesan', 'Data Berhasil Ditambahkan');
    }

    public function edit($id)
    {
        if (!$this->Skor->detailData($id)) {
            abort(404); 
        }
        $data = [
            'skor' => $this->Skor->detailData($id)
        ];
    ...
    public function update($id)
    {
        ...
        $this->Skor->editData($id, $data);
        return redirect()->route('skor')->with('pesan', 'Data Berhasil Diupdate');
    }

    public function delete($id)
    {
        $this->Skor->deleteData($id);
        return redirect()->route('skor')->with('pesan', 'Data Berhasil Dihapus');
    }

    public function export()
    {
        return Excel::download(new SkorExport, 'skor.xlsx');
    }

    public function import(Request $request)
    {
        Excel::import(new SkorImport, $request->file);
        
        return redirect('/skor')->with('status', 'Data Berhasil Diimport!');
    }
    ...
    public function databergolong(){
        ...
        for($i = 0; $i < $kelas; $i++){
            ...
            // $frekuensi[$i] = Skor::where(function, $query){
            //     $query->select(DB::raw('SUM(frekuensi) as tabel1'))
            //             ->
            // }          
            $frekuensi[$i] = Skor::select(DB::raw('count(*) as frekuensi, skor'))
                                    ->where([
                                        ['skor', '>=', $batasBawah],
                                        ['skor', '<=', $batasAtas],
                                    ])
   ...
   public function chikuadrat(){
        ...
        $n = Skor::count('skor');
        $rata2 = number_format(Skor::average('skor'), 2);
   ...
   public function lilliefors(){               
        ...
        for($i = 0; $i < $n; $i++){
            $frekuensi[$i] = Skor::select('skor', DB::raw('count(*) as frekuensi'))  //ambil skor, hitung banyak skor taruh di tabel frekuensi
                                ->groupBy('skor')    //urutkan sesuai skor
                                ->get();     
        ...
        return view('normlil', ['frekuensi' => $frekuensi, 
    ...
    public function storeMoment(Request $request){       
        ...
        [            
            'x'      =>  'required|numeric|min:1|max:100',
            'y'      =>  'required|numeric|min:1|max:100'
        ],
        [
            'x.min'  =>  'Kolom Skor Hanya Bisa Diisi Angka 1-100',
            'x.max'  =>  'Kolom Skor Hanya Bisa Diisi Angka 1-100',
            'x.numeric' => 'Kolom Hanya Bisa Berisi Angka!',
            'y.min'  =>  'Kolom Skor Hanya Bisa Diisi Angka 1-100',
            'y.max'  =>  'Kolom Skor Hanya Bisa Diisi Angka 1-100',
            'y.numeric' => 'Kolom Hanya Bisa Berisi Angka!'
        ]);
        ...
        return redirect('/korelasiMoment')->with('status', 'Data Berhasil Tambah');

   }

    public function deleteMoment($id)
    {
       $moment = Moment::find($id);         //cari id yang dipencet       
       $moment->delete();                  //delete id tersebut
    ...
    public function exportmoment(){
        ...
        return Excel::download(new MomentExport, time().'_'.'korelasimoment.xlsx');               
    }
   ...
   public function korelasiBiserial(){
    ...
    public function exportbiserial(){
        ...
        return Excel::download(new BiserialExport, time().'_'.'korelasibiserial.xlsx');               
    }
